...
namespace Pede_RocaAPP.API.Controllers
{
    ...
    public class MensagemController : ControllerBase
    {
        ...
        {
            if (mensagemDTO == null) return BadRequest("Erro de dados inválidos. Verifique o payload de envio e tente novamente!");
        ...
        public async Task<ActionResult> Put(Guid id, [FromBody] MensagemUpdateDTO mensagemDTO)
        {
            ...
            if (!string.IsNullOrEmpty(mensagemDTO.Email)) mensagemEncontrada.Email = mensagemDTO.Email;
            if (!string.IsNullOrEmpty(mensagemDTO.Conteudo)) mensagemEncontrada.Conteudo = mensagemDTO.Conteudo;
            if (!string.IsNullOrEmpty(mensagemDTO.UidAnexo)) mensagemEncontrada.UidAnexo = mensagemDTO.UidAnexo;
            if (!string.IsNullOrEmpty(mensagemDTO.Status)) mensagemEncontrada.Status = mensagemDTO.Status;
            
            await _mensagemService.AtualizarAsync(id, mensagemEncontrada);
            return Ok(new { mensagem = $"Mensagem com o id {id} foi atualizada com sucesso" });
        }
        ...
        public async Task<ActionResult<MensagemDTO>> Get(Guid id)
        {
            ...
            if (mensagemDto == null) return NotFound("Mensagem não encontrada");
        ...
        public async Task<ActionResult<IEnumerable<MensagemDTO>>> Get()
        {
            ...
            if (mensagens == null) return Ok(new List<MensagemDTO>());
        ...
        public async Task<ActionResult<MensagemDTO>> DeleteAsync(Guid id)
        {
            ...
            return Ok(new { message = "Mensagem removida com sucesso" });
        }
    }
}